...
export default function ForecastGroup() {
    // const { response, error, loading } = useAxios("12.157383", "55.613262")
    let response = {
        "cod": "200",
        "message": 0,
        "cnt": 40,
    ...
    return (
        <div className="relative overflow-hidden rounded-[3rem] backdrop-blur-2xl">
            ...
            <div className="flex gap-2 px-4 pt-16 pb-28 rounded-[3rem] overflow-x-scroll">
                {response.list.map(object => (
                    <SmallForecast key={object.dt} 
                        temp={Math.round(object?.main?.temp - kevin)} 
                        icon={"/icons/" + object.weather[0].icon + ".svg"}
                        rain={object.rain && Math.round(object?.rain["3h"] * 100)} 
                        time="Now" focus="true"/>
                ))}
            </div>
        </div>
